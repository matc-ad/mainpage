...
import { colorsOptions, formulariContacte } from "../constants";
...
const Header = ({ isDarkMode }) => {
	...
	return (
		...
		>
			...
			>
				<h1 className={`text-5xl font-extrabold ${colors.title}`}>
					Benvingut a matc.ad
				</h1>
				<p className={`mt-4 text-2xl ${colors.paragraph}`}>
					Lorem ipsum (no sé què posar aquí) dolor sit amet.
				</p>
				<button
					onClick={handleButtonClick}
					className={`mt-8 font-bold py-2 px-6 rounded-full shadow-lg ${colors.buttonBackground} ${colors.buttonText} ${colors.buttonHoverBackground}`}
				>
					Comença Ara
				</button>
			</div>
